...
def get_example(entry):
    """
    Prefer proper examples with English translation.
    """

    # Pass 1: examples tagged as "example"
    for sense in entry.get("senses", []):
...
def create_database():
    ...
    with open(JSONL_FILE, "r", encoding="utf-8") as f:
        ...
        for line in f:
            ...
            cur.execute("""
            INSERT OR REPLACE INTO adverbs
            (
                word,
                pos,
                meaning,
            ...
            if count % 10000 == 0:
                print(f"Imported {count:,} adverbs...")

    conn.commit()
    conn.close()

    print(f"\nDone. Imported {count:,} adverbs.")
